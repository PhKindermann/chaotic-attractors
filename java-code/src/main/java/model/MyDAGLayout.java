/*
 * EDIT 2021/10/18: set source worked hard sometimes; added break condition
 *
 *
 *
 ...
 * @author John Yesberg
 */
public class MyDAGLayout<V, E> extends MySpringLayout<V,E> {
    ...
    public MyDAGLayout(Graph<V,E> g) {
        super(g);
    }
    ...
    public void propagateMinimumLevel(V v) {
        ...
        for(V child : getGraph().getPredecessors(v)) {
            ...
            if (o != null) {
                ...
                oldLevel = 0;
                minLevels.put(child, new Integer(0));
            }
    ...
    protected void moveNodes() {
        ...
        synchronized (getSize()) {
            ...
            for(V v : getGraph().getVertices()) {
                if (isLocked(v))
                    continue;
                MySpringLayout.SpringVertexData vd = springVertexData.getUnchecked(v);
                Point2D xyd = apply(v);
